...
If you are dealing with multiple, say 100 columns, and like to select 26th to 75th columns, you can do so by using `:` or range function.
...
Here, we use a new dataset, *iris* for demonstration of helper functions.
...
## matches is like a grep and can also matches for a regular expression or wildcards
iris %>%
  select(matches("Wi.*th")) %>%
  head(2)

## extract based on character verctor of column names
colnames_to_extract <- c("Sepal.Length", "Species")

iris %>%
  select(one_of(colnames_to_extract)) %>%
  head(2)

## we can even arrange columns
iris %>%
  select(one_of(colnames_to_extract), everything()) %>%
  head(2)
```

You can learn more about additional helper functions `?tidyselect::select_helpers`
